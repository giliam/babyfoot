...
\begin{itemize}
    ...
    \item Partie serveur
\end{itemize}
La partie cliente sera elle-même séparée en plusieurs modules, ce qui n'est pas nécessaire pour le client.
\begin{itemize}
...
\paragraph{}
Des messages sont échangés entre le client et le serveur et ce de façon répétée. Il faut donc utiliser des Thread pour gérer l'émission et la réception de données depuis le serveur et un Thread pour gérer la réception côté client. Le serveur se chargera ensuite d'enregistrer les données statiques 
...
\paragraph{Game.java}
Contient les données principales pour une partie, notamment l'état des scores, les joueurs y participant, l'avancement de la partie, etc. Sera appelée par le serveur.
